...
def load_model(cfg):
    ...
    return model
def class_road(config_file, input_img, output_img, scale = 1.0, opts = []):
    # Create config
    ...
    color_mask[np.squeeze(mask == 1, -1), :] = (0, 0, 255)  # river
...
def main() -> int:
    ...
    class_road(args.config_file, args.input_img, args.output_img, args.scale, args.opts)
    return 0
