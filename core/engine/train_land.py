...
def draw_masks(image, classes_mask):
    ...
    color_mask[classes_mask == 1, :] = (255, 255, 0)  # (255, 128, 0) # urban_land
    color_mask[classes_mask == 2, :] = (0, 255, 255)  # (0, 255, 239) # agriculture_land
    color_mask[classes_mask == 3, :] = (255, 0, 255)  # (0, 255, 239) # rangeland
    color_mask[classes_mask == 4, :] = (0, 255, 0)  # (0, 255, 239) # forest_land
    color_mask[classes_mask == 5, :] = (255, 0, 0)  # (0, 255, 239) # water
    color_mask[classes_mask == 6, :] = (255, 255, 255)  # (0, 255, 239) # barren_land
    color_mask[classes_mask == 7, :] = (0, 0, 0)  # (0, 255, 239) # unknown
...
def do_train(cfg,
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_ITER):
        ...
        for iteration, data_entry in pbar:
            ...
            jaccard_losses = criterion_jaccard.forward(torch.stack(outputs_classes, dim=1),
                                                       torch.stack(target_classes, dim=1), masks);
            dice_losses = criterion_dice.forward(torch.stack(outputs_classes, dim=1),
                                                 torch.stack(target_classes, dim=1), masks)
            ################### Best images
            if cfg.TENSORBOARD.BEST_SAMPLES_NUM > 0:
                ...
                with torch.no_grad():
                    ...
                    if need_save:
                        ...
                        best_output = torch.stack([best_output, best_output, best_output], dim=0);
                        ...
                        save_img = torch.cat([save_img, best_label, best_output], dim=2);
            ...
            if cfg.TENSORBOARD.WORST_SAMPLES_NUM > 0:
                with torch.no_grad():
                    ...
                    if need_save:
                        ...
                        worst_output = torch.stack([worst_output, worst_output, worst_output], dim=0);

                        worst_image = worst_image.astype(np.float32) / 255.0
                        save_img, _, _ = totensor(worst_image)
                        save_img = torch.cat([save_img, worst_label, worst_output], dim=2);
        ...
        if args.eval_step > 0 and epoch % args.eval_step == 0:
            ...
            result_dict['loss'], result_dict['jaccard'], result_dict['dice']))
        ...
        if epoch % args.save_step == 0:
            ...
            if summary_writer:
                with torch.no_grad():
                    ...
                    summary_writer.add_scalar('metrics/jaccard', jaccard_loss_sum / (iteration + 1),
